...
import { JournalInlineDocument, JournalInlineDraft, editableJournalSections, sectionValue } from '../../data/journalInline';
...
export class JournalInlineEditor {
	...
	async mount(parent: HTMLElement, kind: JournalKind): Promise<void> {
		const source = await this.doc.read();
		if (this.disposed) return;
		if (kind === 'day') this.field(parent, { kind: 'title' }, source, '', '给今天一个标题…', true);
		for (const title of editableJournalSections(kind)) {
			...
			} else this.field(section, { kind: 'section', title }, source, title, kind === 'day' ? title === '今日日记' ? '今天主要……' : '今天值得记住……' : '在这里写下你的复盘……');
		}
	}
	private field(parent: HTMLElement, field: EditorField, source: string, label: string, placeholder: string, title = false): void {
		...
		const input = row.createEl('textarea', { cls: 'mx-journal-inline-input', attr: { rows: '1', placeholder, 'aria-label': label || '日记标题', 'data-journal-field': field.kind === 'section' ? field.title : field.kind } });
		...
		const grow = () => { input.style.height = 'auto'; input.style.height = `${Math.min(Math.max(input.scrollHeight, 38), 480)}px`; };
		...
		if (field.kind === 'quick') { const append = footer.createEl('button', { cls: 'mx-inline-action', text: '记下', attr: { type: 'button' } }); append.onclick = () => { void draft.flush(true); }; }
	}
	...
	async destroy(): Promise<boolean> { const ok = await this.flush(); if (ok) this.disposed = true; return ok; }
}
