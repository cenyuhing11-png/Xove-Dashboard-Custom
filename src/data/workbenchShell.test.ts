...
test('Seven shared navigation labels remain in the approved order',()=>{
	const f=fixture(),s=f.shell();assert.deepEqual(s.root.querySelectorAll('.ad-toolbar__btn').map(e=>e.textContent),['首页','进程','收件箱','新建日记','新建任务','新建项目','▦更多工具']);s.instance.unload();
});
