...
test('Seven shared navigation labels remain in the approved order',()=>{
	const f=fixture(),s=f.shell();assert.deepEqual(s.root.querySelectorAll('.ad-toolbar__btn').map(e=>e.textContent),['首页','进程','收件箱','新建日记','新建任务','新建进程','▦更多工具']);s.instance.unload();
});
...
test('New list presentation preserves learning and journal selectors and source task completion',()=>{
	...
	const tasks=readFileSync(new URL('../views/EmbeddedTaskModal.ts',import.meta.url),'utf8');assert.ok(tasks.includes('store.complete(task, check.checked)'));assert.ok(tasks.includes('this.store.bySource(this.path)'));assert.ok(tasks.includes('this.unsubscribe = undefined'));
});
