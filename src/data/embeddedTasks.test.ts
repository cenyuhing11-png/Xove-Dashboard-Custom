...
test('all tasks grouped without loss', () => { const tasks = [first(), first('## 项目任务\n- [x] 完成', project), first('## 日常待办\n- [ ] 日常', DAILY_TASK_FILE)]; const groups = groupEmbedded(tasks); assert.deepEqual(Object.keys(groups), ['project', 'learning', 'daily']); assert.equal(Object.values(groups).flat().length, 3); });
function memory() {
...
test('toolbar routes to embedded modal while legacy modal remains', () => { const source = readFileSync(new URL('../views/DashboardView.ts', import.meta.url), 'utf8'); assert.ok(source.includes("if (it.action === 'task') new NewEmbeddedTaskModal")); assert.ok(source.includes('private async openTaskModal(')); });
