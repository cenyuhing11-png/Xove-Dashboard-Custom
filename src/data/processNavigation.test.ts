...
test('Home/process/inbox use the same LifeCompass renderer, config and classes',()=>{
	...
	assert.equal(home.includes('renderLifeCompass('),false);assert.ok(view.includes('renderLifeCompass('));assert.equal(dashboard.match(/renderLifeCompass\(/g)?.length,1);assert.ok(dashboard.includes('private lifeCompass?'));assert.equal(view.includes('wb-compass__lane'),false);assert.equal(dashboard.includes('wb-compass__lane'),false);
