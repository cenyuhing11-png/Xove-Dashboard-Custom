...
test('default project toolbar and homepage use formal scanner/view',()=>{const view=readFileSync(new URL('../views/DashboardView.ts',import.meta.url),'utf8');assert.ok(view.includes("if (action === 'project') new UnifiedProcessModal"));assert.ok(view.includes("action === 'plan' || action === 'all'"));assert.ok(view.includes('navigateWorkbench(action, this.leaf)'));assert.ok(view.includes('scanProjects(this.app)'));});
