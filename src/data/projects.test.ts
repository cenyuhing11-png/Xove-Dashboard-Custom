...
test('formal projectsFolder default and task picker wired through Process Adapter',()=>{const settings=readFileSync(new URL('../settings.ts',import.meta.url),'utf8');assert.ok(settings.includes('projectsFolder: PROJECT_ROOT'));const picker=readFileSync(new URL('../views/EmbeddedTaskModal.ts',import.meta.url),'utf8');assert.ok(picker.includes('processes(scanLearning(this.app), scanProjects(this.app), [])'));assert.equal(picker.includes('getMarkdownFiles().filter'),false);});
test('default project toolbar and homepage use formal scanner/view',()=>{const view=readFileSync(new URL('../views/DashboardView.ts',import.meta.url),'utf8');assert.ok(view.includes("action === 'project') new UnifiedProcessModal"));assert.ok(view.includes("action === 'all') await this.setSection('process')"));assert.ok(view.includes('new ProjectBoard(this.processSource)'));assert.ok(view.includes('scanProjects(this.app)'));});
