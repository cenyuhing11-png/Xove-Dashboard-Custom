...
test('default project toolbar and homepage use formal scanner/view',()=>{const view=readFileSync(new URL('../views/DashboardView.ts',import.meta.url),'utf8');assert.ok(view.includes("if (action === 'project') new UnifiedProcessModal"));assert.ok(view.includes("if (action === 'all') await openProjects"));assert.ok(view.includes('scanProjects(this.app)'));});
