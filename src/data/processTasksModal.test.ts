...
test('Quick preview styles native task inputs with original ProjectBoard checkbox class',async()=>{const f=await fixture();assert.equal(f.root.querySelectorAll('.po-check').length,3);assert.ok(f.root.querySelectorAll('input').every(e=>e.classes.has('po-check')));f.modal.close();});
...
test('Compact dimensions and checkbox overrides are scoped away from all details',()=>{const css=readFileSync(new URL('../../styles.css',import.meta.url),'utf8');assert.ok(/\.modal\.ad-propmenu\.mx-quick-tasks-modal \{[^}]*width: 400px;[^}]*max-width: calc\(100vw - 24px\)/.test(css));assert.ok(css.includes('.mx-quick-tasks input.po-check:checked { background: var(--ad-accent);'));assert.ok(css.includes('.mx-quick-tasks input.po-check:focus-visible'));});
