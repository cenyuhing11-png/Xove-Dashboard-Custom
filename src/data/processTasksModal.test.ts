...
async function fixture(type:'learning'|'project'='learning',empty=false){
	...
	class Modal {app:any;contentEl=new Element();containerEl=new Element();modalEl=new Element();constructor(a:any){this.app=a;}open(){(this as any).onOpen();}close(){(this as any).onClose();}}
...
test('Pending task group is expanded with all pending rows',async()=>{const f=await fixture();const pending=f.root.querySelector('section')!;assert.ok(pending.textContent.includes('待完成 2'));assert.equal(pending.querySelectorAll('input').length,2);f.modal.close();});
...
test('Checkbox refresh moves a task into the completed group without opening detail',async()=>{const f=await fixture();const check=f.root.querySelectorAll('input')[0]!;check.checked=true;check.onchange();await flush();assert.ok(f.root.querySelector('section')!.textContent.includes('待完成 1'));assert.ok(f.root.querySelector('summary')!.text.includes('已完成 2'));assert.equal(f.full(),0);assert.equal(f.root.querySelector('details')!.open,false);f.modal.close();});
...
test('Quick tasks have bounded internal scrolling and fixed outer sections',()=>{const css=readFileSync(new URL('../../styles.css',import.meta.url),'utf8');assert.ok(/\.ad-task-modal\.mx-quick-tasks \{[^}]*overflow: hidden/.test(css));assert.ok(/\.mx-quick-tasks__body \{[^}]*min-height: 0;[^}]*overflow-y: auto/.test(css));assert.ok(css.includes('.mx-quick-tasks > :not(.mx-quick-tasks__body) { flex-shrink: 0; }'));assert.ok(css.includes('max-height: min(480px, calc(100dvh - 32px))'));});
test('Compact preview uses original small menu container without replacing Modal lifecycle',async()=>{const f=await fixture();assert.ok(f.modal.modalEl.classes.has('ad-propmenu'));assert.ok(f.modal.modalEl.classes.has('mx-quick-tasks-modal'));f.modal.close();assert.equal(f.modal.modalEl.classes.has('mx-quick-tasks-modal'),false);});
test('Quick preview removes update-log cards from both task groups',async()=>{const f=await fixture();assert.equal(f.root.querySelectorAll('.ad-update-block').length,0);assert.equal(f.root.querySelectorAll('section').length,1);assert.equal(f.root.querySelectorAll('details').length,1);f.modal.close();});
test('Type and progress share one compact summary below the process title',async()=>{const f=await fixture();assert.equal(f.root.querySelectorAll('h3').length,1);assert.equal(f.root.querySelectorAll('.ad-modal-hint').length,1);assert.equal(f.root.querySelector('.ad-modal-hint')!.text,'学习 · 任务进度：1 / 3');f.modal.close();});
test('Quick preview receives the shared native controls and original ProjectBoard visuals',async()=>{const f=await fixture();assert.equal(f.root.querySelectorAll('.po-check').length,3);assert.equal(f.root.querySelectorAll('.mx-embedded-task-checkbox').length,3);assert.ok(f.root.querySelectorAll('input').every(e=>e.classes.has('mx-embedded-task-check')));f.modal.close();});
test('Footer reuses lightweight original back button without large primary action',async()=>{const f=await fixture();const footer=f.root.querySelector('.ad-modal-btns')!;assert.equal(footer.querySelector('.po-back-btn')!.text,'打开完整详情 →');assert.equal(footer.querySelector('.ad-modal-btn'),undefined);f.modal.close();});
test('Compact dimensions remain while checkbox visuals come only from the shared component',()=>{const css=readFileSync(new URL('../../styles.css',import.meta.url),'utf8');assert.ok(/\.modal\.ad-propmenu\.mx-quick-tasks-modal \{[^}]*width: 400px;[^}]*max-width: calc\(100vw - 24px\)/.test(css));assert.equal(css.includes('.mx-quick-tasks input.po-check'),false);});
