...
test('narrative menus expose edit only and retain existing Markdown writer',()=>{const source=readFileSync(new URL('../views/PlanView.ts',import.meta.url),'utf8');const menu=source.slice(source.indexOf('menuButton.onclick'),source.indexOf("const stages=main.createDiv"));assert.match(menu,/编辑内容/);assert.doesNotMatch(menu,/删除|重命名|上移|下移/);assert.match(menu,/updateLongTermPlanMarkdown/);assert.match(menu,/updateNarrativeMarkdown/);});
