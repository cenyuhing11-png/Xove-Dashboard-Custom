...
export class PlanWorkspaceRenderer extends Component {
	...
	private renderLongTermDetail(main: HTMLElement, plan: LongTermPlan, plans: LongTermPlan[]): void {
		...
		for(const stage of plan.stages){
			...
			const meta=row.createDiv({cls:'mx-long-term-stage-meta',text:refs.length?`${refs.length} 个进程`:'暂无进程'});
			const details=row.createDiv({cls:'mx-long-term-stage-details'});details.hidden=!expanded;
			toggle.onclick=()=>{const next=details.hidden;details.hidden=!next;row.toggleClass('is-expanded',next);toggle.textContent=next?'⌄':'›';toggle.setAttribute('aria-expanded',String(next));toggle.setAttribute('aria-label',`${next?'收起':'展开'}阶段 ${stage.text}`);if(next)this.expandedLongTermStageIds.add(stage.id);else this.expandedLongTermStageIds.delete(stage.id);};
			const note=details.createDiv({cls:'mx-long-term-stage-note'});note.createDiv({cls:'ad-modal-label',text:'这一阶段重点'});this.renderLongTermMarkdown(note,stage.note,plan.path,'尚未填写阶段说明');
